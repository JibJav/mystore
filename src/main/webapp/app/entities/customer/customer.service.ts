...
export class CustomerService {
  ...
  protected convertDateFromClient(customer: ICustomer): ICustomer {
    const copy: ICustomer = Object.assign({}, customer, {
      timestamp: customer.timestamp && customer.timestamp.isValid() ? customer.timestamp.toJSON() : undefined,
    });
    return copy;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((customer: ICustomer) => {
        customer.timestamp = customer.timestamp ? moment(customer.timestamp) : undefined;
      });
    }
    return res;
  }
